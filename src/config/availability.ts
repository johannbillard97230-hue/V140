...
// August 2026 availability: 1-3, 5 (blocks 7-10, 11-13, 28-31)
export const augustAllowedRanges = [
  { from: new Date(2026, 7, 1), to: new Date(2026, 7, 3) },
  { from: new Date(2026, 7, 5), to: new Date(2026, 7, 5) },
];

// September 2026 availability: 1, 6, 20-21, 24, 30 (blocks 2-5, 7-12, 13-19, 22-23, 25-29)
export const septemberAllowedRanges = [
  ...
  { from: new Date(2026, 8, 20), to: new Date(2026, 8, 21) },
  { from: new Date(2026, 8, 24), to: new Date(2026, 8, 24) },
...
export function isDateAllowed(date: Date): boolean {
  ...
  // November 2026: block 1-2
  if (date.getFullYear() === 2026 && date.getMonth() === 10) {
    const day = date.getDate();
    if (day >= 1 && day <= 2) return false;
    return true;
  }

  // December 2026 and beyond: all dates open
  return true;
}
